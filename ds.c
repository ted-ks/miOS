...
uint8_t kernel_queue_is_empty(struct kernel_queue *kq)
{
...
uint8_t kernel_queue_is_full(struct kernel_queue *kq)
{
    int16_t diff = kq->kq_in - kq->kq_out;
...
uint8_t kernel_queue_pop(struct kernel_queue *kq)
{
    uint8_t return_data;
...
uint8_t kernel_queue_push(struct kernel_queue *kq, uint8_t data)
{
    if(kq->is_kq_full(kq))
        return 1;
